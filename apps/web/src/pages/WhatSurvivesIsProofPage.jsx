...
export default function WhatSurvivesIsProofPage() {
  const book = bookPages.find(b => b.id === 'what-survives-is-proof');
  ...
  return (
    <div className={`archive-page flex min-h-screen book-detail ${book.styleClass || ''}`}>
      ...
      <style>{`
        .book-evidence {
          --background: 210 20% 98%; /* hospital gray */
          --foreground: 220 15% 20%; /* ledger blue undertone */
          background-color: hsl(var(--background));
          color: hsl(var(--foreground));
        }
        .book-evidence .metadata-grid .catalogue-format-key {
          color: #475569; /* slate */
        }
        .book-evidence .metadata-grid .catalogue-format-value {
          color: #0f172a; /* deep slate */
          font-weight: 600;
        }
        .book-evidence .book-positioning {
          border-left-color: #64748b; /* slate border */
          background: linear-gradient(90deg, rgba(100, 116, 139, 0.05) 0%, transparent 100%);
          padding: 1rem 1.5rem;
        }
        .book-evidence .quote-purpose {
          color: #475569;
        }
      `}</style>
